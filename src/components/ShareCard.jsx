...
import { fmtLong, calcVol, kgToLb, bestRM, isCardioEx } from '../utils';
...
export default function ShareCard({workout:w,c,unit="kg",onClose,bwKg=0}){
  ...
  const shareText=()=>{
    var lines=[" "+w.name+" — "+fmtLong(w.date),""];
    w.exercises.forEach(ex=>{
      const isC=ex.isCardio!=null?!!ex.isCardio:isCardioEx(ex.name,ex.muscle);
      const setStr=isC
        ?ex.sets.map(s=>(s.mins||'–')+"min"+(s.dist?" "+s.dist+"km":"")).join(", ")
        :ex.sets.map(s=>(s.bodyweight?"BW":s.weight+unit)+"×"+s.reps).join(", ");
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"linear-gradient(140deg,#1a1a2e,#16213e,#0f3460)",borderRadius:28,padding:"30px 22px",width:"100%",maxWidth:320,boxShadow:"0 24px 80px rgba(0,0,0,0.8)"}}>
        <div style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:36,marginBottom:6}}></div><div style={{fontSize:22,fontWeight:900,color:"#fff",letterSpacing:"-0.02em"}}>{w.name}</div><div style={{fontSize:12,color:"#8888bb",marginTop:4}}>{fmtLong(w.date)}</div></div>
