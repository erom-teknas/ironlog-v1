...
function App(){
  ...
  const repeatW=w=>{
    const doRepeat=()=>{
      ...
      setLogName(w.name+" (repeat)");draftT0.current=Date.now();draftWorkoutT0.current=null;setTab("log");
    };
  ...
  useEffect(()=>{idbGet("il_custom_routines",[]).then(r=>setCustomPlans(r||[]));;},[]);
  useEffect(()=>{if(loaded)idbSet("il_custom_routines",customPlans);},[customPlans,loaded]);
  const saveCustomPlan=(r)=>{
    setCustomPlans(p=>{const idx=p.findIndex(x=>x.id===r.id);return idx>=0?p.map(x=>x.id===r.id?r:x):[...p,r];});
  };
  const deleteCustomPlan=(id)=>setCustomPlans(p=>p.filter(r=>r.id!==id));
  ...
  // Custom exercise input types — flat name→type map, separate from names list
  const [customExTypes,setCustomExTypes]=useState({});
  useEffect(()=>{idbGet("il_custom_ex_types",{}).then(r=>setCustomExTypes(r||{}));},[]);
  useEffect(()=>{if(loaded)idbSet("il_custom_ex_types",customExTypes);},[customExTypes,loaded]);
  const addCustomExercise=(muscle,name,inputType)=>{
    var n=name.trim();
    if(!n)return;
    ...
    if(inputType&&inputType!=="weighted"){
      setCustomExTypes(p=>({...p,[n]:inputType}));
    }
  };
  const deleteCustomExercise=(muscle,name)=>{
    setCustomExercises(p=>({...p,[muscle]:(p[muscle]||[]).filter(x=>x!==name)}));
    setCustomExTypes(p=>{const next={...p};delete next[name];return next;});
  };
  const renameCustomExercise=(muscle,oldName,newName)=>{
    ...
    setCustomExTypes(p=>{
      if(!p[oldName])return p;
      const next={...p,[n]:p[oldName]};delete next[oldName];return next;
    });
  };
  ...
  const applyRawBackup=useCallback((raw)=>{
    ...
    const doReplace=()=>{
      setHist(sanitized);
      if(importedPlans)setCustomPlans(importedPlans);
      if(importedBw)setBwLog(importedBw);
      if(importedEx)setCustomExercises(importedEx);
      if(importedExTypes)setCustomExTypes(importedExTypes);
      if(importedMeas)setMeasLog(importedMeas);
    };
    if(hist.length===0){
      doReplace();
    ...
    return true;
  },[hist,customPlans,bwLog,customExercises,customExTypes,showImportDialog]);
  ...
  const restoreFromSnapshot=useCallback((snap)=>{
    ...
    setHist(snap.workouts);
    if(snap.customPlans&&snap.customPlans.length)setCustomPlans(snap.customPlans);
    if(snap.bwLog&&snap.bwLog.length)setBwLog(snap.bwLog);
    if(snap.customExercises&&Object.keys(snap.customExercises).length)setCustomExercises(snap.customExercises);
    if(snap.customExTypes&&Object.keys(snap.customExTypes).length)setCustomExTypes(snap.customExTypes);
  },[]);
