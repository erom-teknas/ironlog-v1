import { PLATES_LB, PLATES_KG, BAR_LB, BAR_KG, EX_TYPES } from './constants.js';
...
export function getStreak(hist,restDays){var rd=typeof restDays==="number"?restDays:1;if(!hist.length)return 0;var dates=[...new Set(hist.map(w=>w.date))].sort().reverse();var s=0,cur=new Date(today());for(var i=0;i<dates.length;i++){var diff=Math.round((cur-new Date(dates[i]))/86400000);if(diff<=(rd+1)){s++;cur=new Date(dates[i]);}else break;}return s;}
...
export function calcPlates(t,unit,barKgOverride,customPlatesKg){
  // Only use user-defined plates — never fall back to hardcoded lists.
  // If the user hasn't set any plates yet, return [] (no plates shown).
  if(!customPlatesKg||customPlatesKg.length===0)return[];
  var sorted=[...customPlatesKg].sort((a,b)=>b-a);
  var PLATES=unit==="lb"?sorted.map(p=>Math.round(kgToLb(p)*4)/4):sorted;
...
// ── Exercise input-type helpers — shared by LogPage, HistoryPage, ProgressPage ─
// getExInputType: returns "weighted"|"bodyweight"|"cardio"|"timed"
// Checks customExTypes first, then EX_TYPES lookup, then legacy heuristics.
export function getExInputType(name,muscle,customExTypes){
  if(customExTypes&&customExTypes[name])return customExTypes[name];
  if(EX_TYPES[name])return EX_TYPES[name];
  // Legacy heuristics for untyped exercises (backward compat)
  var n=(name||"").toLowerCase();
  var m=(muscle||"").toLowerCase();
  if(m.includes("cardio")||m.includes("hiit")||m.includes("plyom"))return"cardio";
  if(n.includes("run")||n.includes("jog")||n.includes("bike")||n.includes("cycl")||
     n.includes("treadmill")||n.includes("elliptic")||n.includes("stair")||
     n.includes("swim")||n.includes("rowing")||n.includes("rower")||
     (n.includes("walk")&&!n.includes("lunge")&&!n.includes("farmer")&&!n.includes("carry"))||
     n.includes("skierg")||n.includes("ski erg")||n.includes("assault bike"))return"cardio";
  return"weighted";
}
export function isTimedEx(name,customExTypes){
  if(customExTypes&&customExTypes[name])return customExTypes[name]==="timed";
  return EX_TYPES[name]==="timed";
}
// isCardioEx — kept for backward compat with stored ex.isCardio flags
export function isCardioEx(name,muscle,customExTypes){
  return getExInputType(name,muscle,customExTypes)==="cardio";
}
